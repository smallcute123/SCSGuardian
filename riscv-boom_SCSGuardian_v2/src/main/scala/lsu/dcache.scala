    ...
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            ...
                            !(io.lsu.exception && s2_req(w).uop.uses_ldq)   &&
                             (isPrefetch(s2_req(w).uop.mem_cmd) ||
                              ...
                              isWrite(s2_req(w).uop.mem_cmd)) && !((s2_req(w).uop.br_mask =/= 0.U || io.lsu.unsafe_mem((s2_req(w).uop.ldq_idx))) &&  s2_req(w).uop.uses_ldq && io.lsu.reset)
...
//[SCSGuardian] modified
    when( s2_valid(w)          &&
      ...
      (isPrefetch(s2_req(w).uop.mem_cmd) ||
       ...
       (((s2_req(w).uop.br_mask =/= 0.U)||io.lsu.unsafe_mem((s2_req(w).uop.ldq_idx))) && s2_req(w).uop.uses_ldq && io.lsu.reset))
    {
