...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  //[SCSGuardian] add
  val unsafe_mem = Output(Vec(numRobRows,Bool()))
...
//[SCSGurdian] for unsafe tlb miss request
    ...
    dtlb.io.load_risk(w)             := (will_fire_load_incoming(w) || will_fire_load_retry(w)) && exe_tlb_uop(w).uses_ldq &&  ((exe_tlb_uop(w).br_mask =/= 0.U)&& io.dmem.reset)
  }
  ...
  for (w <- 0 until memWidth) {
    ...
    io.core.clr_unsafe(w).bits  := RegNext(lcam_uop(w).rob_idx)
     //[SCSGuardian]Determine whether the current load has an address conflict with the uncommitted store instruction
      ...
      //  io.dmem.unsafe_mshr := (0 until numLdqEntries).map{ i =>ldq(i).valid && (!ldq(i).bits.addr.valid)}.reduce(_||_) =/=0.U
