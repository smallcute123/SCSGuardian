  ...
  with HasBoomCoreParameters
{
  ...
  mshrs.io.unsafe_mshr:=io.lsu.unsafe_mshr
  mshrs.io.unsafe_mem := io.lsu.unsafe_mem
  mshrs.io.reset := io.lsu.reset
  ...
  // unsafe_tempmshr := false.B
  // Miss handling
  for (w <- 0 until memWidth) {
    // when{unsafe_tempmshr===true.B}{
    //   unsafe_mshr(w)==true.B

    // }.otherwise{
    //   unsafe_mshr(w)==false.B
    // }
    /*xiaoyu-SCSGuardian*/
    mshrs.io.req(w).valid := s2_valid(w)          &&
                            ...
                            !(io.lsu.exception && s2_req(w).uop.uses_ldq)   &&
                             (isPrefetch(s2_req(w).uop.mem_cmd) ||
                              ...
                              isWrite(s2_req(w).uop.mem_cmd)) && !(((s2_req(w).uop.br_mask =/= 0.U||io.lsu.unsafe_mem((s2_req(w).uop.ldq_idx)))&& s2_req(w).uop.uses_ldq && io.lsu.unsafe_mshr((s2_req(w).uop.ldq_idx))) && io.lsu.reset)

  //  when(mshrs.io.req(w).valid === 1.U && s2_req(w).uop.uses_ldq){
  //   unsafe_tempmshr := true.B
  //  }
...
//[SCSGuardian] modified
    when( s2_valid(w)          &&
      ...
      (isPrefetch(s2_req(w).uop.mem_cmd) ||
       ...
       (((s2_req(w).uop.br_mask =/= 0.U || io.lsu.unsafe_mem((s2_req(w).uop.ldq_idx))) &&  s2_req(w).uop.uses_ldq && io.lsu.unsafe_mshr((s2_req(w).uop.ldq_idx))) && io.lsu.reset))
    {
