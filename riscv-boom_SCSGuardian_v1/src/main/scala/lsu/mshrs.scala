  ...
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    ...
    // val unsafe_mshr =Input(Bool())
    val unsafe_mshr =Input(Vec(numLdqEntries,Bool()))
    // val unsafe_mem =Input(Bool())
    val unsafe_mem = Input(Vec(numLdqEntries,Bool()))
  ...
  when (state === s_invalid) {
    ...
    when (!(((rpq.io.deq.bits.uop.br_mask =/= 0.U||io.unsafe_mem(rpq.io.deq.bits.uop.ldq_idx))) && !io.unsafe_mshr((rpq.io.deq.bits.uop.ldq_idx)) && rpq.io.deq.bits.uop.uses_ldq && io.reset)){
       commit_line   := true.B
      //  if(UNSAFE_TRACE){
  ...
  with HasL1HellaCacheParameters
{
  ...
  //[SCSGuardian]add
   ...
   mshr.io.unsafe_mshr := io.unsafe_mshr
   mshr.io.unsafe_mem := io.unsafe_mem
   mshr.io.reset := io.reset
