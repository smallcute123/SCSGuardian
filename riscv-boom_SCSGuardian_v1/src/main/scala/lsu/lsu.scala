...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  //[SCSGuardian] add
  //  val unsafe_mshr = Output(Bool())
  val unsafe_mshr = Output(Vec(numLdqEntries,Bool()))
  //  val unsafe_mem = Output(Bool())
  val unsafe_mem = Output(Vec(numLdqEntries,Bool()))
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  //[SCSGuardian]:for unresolved memory access order
  //val unsafe_mem = Output(Vec(memWidth, Valid(UInt(robAddrSz.W))))
  val unsafe_mem = Output(Bool())
  // val really_unsafe = Output(Bool())
  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  //[SCSGuardian] for unsafe tlb miss request
    dtlb.io.store_risk_tlb(w)       := (will_fire_stad_incoming(w) || will_fire_sta_incoming(w) || will_fire_sta_retry(w)) && ((exe_tlb_uop(w).br_mask =/= 0.U) && io.dmem.reset ) && exe_tlb_uop(w).uses_stq && exe_tlb_uop(w).lrs1_rtype === RT_FIX 
    dtlb.io.load_risk(w)             := (will_fire_load_incoming(w) || will_fire_load_retry(w)) && exe_tlb_uop(w).uses_ldq && exe_tlb_uop(w).lrs1_rtype === RT_FIX && ((exe_tlb_uop(w).br_mask =/= 0.U) && io.dmem.reset )  
  }
  ...
  for (w <- 0 until memWidth) {
    ...
    io.core.clr_unsafe(w).bits  := RegNext(lcam_uop(w).rob_idx)
     //[SCSGuardian] Determine whether the current load has an address conflict with the uncommitted store instruction
       io.core.unsafe_mem := (0 until numStqEntries).map{ i => stq(i).valid && (!stq(i).bits.addr.valid ) }.reduce(_||_) =/= 0.U
      //  io.dmem.unsafe_mem := (0 until numStqEntries).map{ i => stq(i).valid && (!stq(i).bits.addr.valid ) }.reduce(_||_) =/= 0.U
       
      //  io.core.really_unsafe := (0 until numLdqEntries).map{ i =>ldq(i).valid && (io.core.unsafe_masked(ldq(i).bits.uop.rob_idx >> log2Ceil(coreWidth)) ) }.reduce(_||_) =/= 0.U

      
  }
   //[SCSGuardian] Determine whether the the current load has possible mshr contention with other loads
  val unsafe_mshr = RegInit(false.B)
  for(i <- 0 until numLdqEntries){
     unsafe_mshr := false.B
    io.dmem.unsafe_mem(i):= (0 until numStqEntries).map{ j => Mux(ldq(i).bits.st_dep_mask(j) === 1.U, stq(j).valid && (!stq(j).bits.addr.valid ),false.B)}.reduce(_||_) =/= 0.U
    for(j<- 0 until i){
        when(ldq(j).bits.executed && (ldq(j).bits.uop.br_mask=/=0.U||io.dmem.unsafe_mem(j))){
        unsafe_mshr := true.B
        }
      }
    io.dmem.unsafe_mshr(i) :=  unsafe_mshr
  }
