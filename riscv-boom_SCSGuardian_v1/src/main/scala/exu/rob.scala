  ...
  )(implicit p: Parameters)  extends BoomBundle
{
  ...
  // val lsu_really_unsafe  = Input(Bool())
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val rob_unsafe_masked = WireInit(VecInit(Seq.fill(numRobRows << log2Ceil(coreWidth)){false.B}))
    //[SCSGuardian] initial rob unsafe mask 
val unsafe_masked = WireInit(VecInit(Seq.fill(numRobRows){false.B}))
val unsafe_masked_temp = WireInit(VecInit(Seq.fill(numRobRows){false.B}))
  ...
  for (w <- 0 until coreWidth) {
    ...
    //[SCSGuardian]add for unresolved branch and memory order(SSB)
    val rob_unsafe_branch = Reg(Vec(numRobRows, Bool()))
    val rob_really_unsafe = Reg(Bool())
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    for (i <- 0 until numRobRows) {
      val br_mask = rob_uop(i).br_mask
       //[SCSGuardian]For unresolved branch
      rob_unsafe_branch(GetRowIdx(rob_uop(i).rob_idx)):= Mux(br_mask === 0.U, 
                                      false.B,true.B)
      //rob_uses_ldq (GetRowIdx(rob_uop(i).rob_idx)):= rob_uop(i).uses_ldq
    ...
    when (will_commit(w)) {
      ...
      rob_unsafe_branch(rob_head):=false.B
    }
    ...
    for (i <- 0 until numRobRows) {
      ...
      //[SCSGuardian]add
       // unsafe_masked(i) := rob_val(i) && (rob_unsafe(i) || rob_exception(i)||rob_unsafe_branch(i)||rob_unsafe_lsu(i))
        // rob_really_unsafe := (0 until i).map{ j =>rob_val(j) && (rob_unsafe_branch(j)|| rob_unsafe_lsu(j)) }.reduce(_||_) =/= 0.U
       unsafe_masked_temp(i) :=rob_val(i) && (rob_unsafe_branch(i)|| io.lsu_unsafe_mem)&& io.reset
       rob_really_unsafe := false.B
      for(j<- 0 until i){
        when(unsafe_masked_temp(j)===1.U){
          rob_really_unsafe := true.B
        }
      }
        // rob_unsafe_lsu(i) := io.lsu_unsafe_mem
        unsafe_masked(i) :=rob_val(i) && (rob_unsafe_branch(i)|| io.lsu_unsafe_mem)&& io.reset &&rob_really_unsafe
        
        // unsafe_masked(i) :=rob_val(i) && (rob_unsafe_branch(i)|| io.lsu_unsafe_mem)&& io.reset
    }
