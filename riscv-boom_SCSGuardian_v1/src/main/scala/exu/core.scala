  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  // construct all of the modules
   val unsafereset = WireInit(true.B)
...
//scapro
//fp_pipeline.io.unsafe_masked := rob.io.unsafe_masked

  // for(iu <- issue_units) {
  //    iu.io.unsafe_masked := rob.io.unsafe_masked
  // }
...
//   "CSRFile is being written to spuriously.")
  ...
  //[SCSGuardian]Passing unsafe signals from lsu unit to rob
  rob.io.lsu_unsafe_mem := io.lsu.unsafe_mem
  // rob.io.lsu_really_unsafe:= io.lsu.really_unsafe
  ...
  assert (!(idle_cycles.value(13)), "Pipeline has hung.")
    ...
    // rob.io.lsu_really_unsafe :=false.B
    unsafereset:=false.B
    }
    // when( idle_cycles.value(6) && idle_cycles.value(2)){
    // rob.io.lsu_unsafe_mem :=false.B
    // // rob.io.lsu_really_unsafe :=false.B
    // unsafereset:=false.B
    // }   
    // when( idle_cycles.value(8) && idle_cycles.value(4)){
    // rob.io.lsu_unsafe_mem :=false.B
    // //  rob.io.lsu_really_unsafe :=false.B
    // unsafereset:=false.B
    // }
    // when( idle_cycles.value(9) && idle_cycles.value(5)){
    // rob.io.lsu_unsafe_mem :=false.B
    // //  rob.io.lsu_really_unsafe :=false.B
    // unsafereset:=false.B
    // }
    // when( idle_cycles.value(12) && idle_cycles.value(10)){
    // rob.io.lsu_unsafe_mem :=false.B
    // // rob.io.lsu_really_unsafe :=false.B
    // unsafereset:=false.B
    // }
    rob.io.reset := unsafereset
    io.lsu.reset := unsafereset
